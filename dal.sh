...
dal() {
	...
	case $1 in
		...
		add)
			...
			local IS_THERE=""
			IS_THERE=$(awk -v alias="${ALIAS}" 'match($1,"^" alias "$" )' $HOME/.dal)
			if [ -z "${IS_THERE}" ]; then
				echo "${ALIAS} ${_PATH}" >> $HOME/.dal
			else
		...
		goto)
			...
			# cd to the path.
			ALIAS="$2"
			_PATH=$(awk -v alias="^${ALIAS}$" '$1 ~ alias{print $0; exit}' $HOME/.dal)
		...
		path)
			...
			_PATH=$(awk -v alias="${ALIAS}" 'match($1, "^" alias "$")' $HOME/.dal)
